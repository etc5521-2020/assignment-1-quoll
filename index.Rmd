...
*Edit*: Redo intro
*Edit*: Refine primary and secondary questions
*Edit*: Find some interesting facts
...
### Total Space Projects and Missions Embarked
...
```{r yearly-mission-duration, fig.width = 8}
...
Although the total number of missions decreased, figure \@ref(fig:yearly-mission-duration) illustrates that the total number of hours increased progressively over the years. This implies that astronauts are spending more time in space. 

This increase can be attributed to the fact that the international space station was completed...

- Was there any breakthrough that cased the icnrease?


## Lead Time Required to Train an Astronaut

```{r First-flights, fig.height = 6}
...
astronauts %>%
  ...
  labs(x = "Year of Mission",
       y = "Total Astronauts",
       title = "Total Number of New Astronauts on Their First Mission") +
  theme_linedraw() +
  our_theme 
```
...
- Train 1 year for a shuttle, 3-4 years for ISS.
- Opening of ISS
  - Annotate the opening of ISS?


### Participating Astronauts' Nationalities

*Edit*
- Instead of only looking at years, we look at each decade and each significant evolution (e.g. Did the completion of international space station attracted more astronauts from around the world?)
  - Facet each decade into panels
- Over the years, are there more astronauts from across the globe?
  - Plot world map to show more participating countries?
...
pp <- astronauts %>%
  ...
  geom_col(aes(x = year_of_mission,
               ...
               fill = nationality),
           alpha = 0.5) +
  scale_fill_manual(values = c("Other" = "#b7ff49", "U.S." = "#92b4f2", "U.S.S.R/Russia" = "#df9797")) +
  theme_linedraw() +
  our_theme +
  theme(panel.grid = element_blank(),
        legend.position = "none") +
  labs(y = "Total Space Missions",
       x = "Year Of Mission",
       title = "Country's Total Space Missions per Year") 

ggplotly(pp)  

# Total Number of participating countries (n_distinct)
astronauts %>%
  group_by(year_of_mission) %>%
  summarise(distinct_countries = n_distinct(nationality)) %>%
  ggplot(aes(x = year_of_mission,
...
> "global astronautical exploration competition between each government is fierce, and the competition has reached the peak in 1985"
...
sample_cmbs <- sample_cmbs %>%
  ...
  mutate(nationality = if_else(condition = nationality %in% c("U.S.S.R/Russia", "U.S."),
         ...
         mission_decade = case_when(
           between(year_of_mission, 1960, 1969) ~ "1960s",
           between(year_of_mission, 1970, 1979) ~ "1970s",
           between(year_of_mission, 1980, 1989) ~ "1980s",
           between(year_of_mission, 1990, 1999) ~ "1990s",
           between(year_of_mission, 2000, 2009) ~ "2000s",
           between(year_of_mission, 2010, 2019) ~ "2010s"
         )
         )
```
...
*1970s*
- Number of space missions (with more than 1 astronaut) increased, however, there are still no collaboration among U.S. and Russian astronauts

*1980s*
- Exponential increase in the number of space missions
- International collaboration initiated: Some collaboration between U.S. and Russia were established
- Other Countries (besides U.S. and Russia) emerged and participated in space missions with the U.S. and Russian astronauts

*1990s*
- 1159 space missions (with more than 1 astronauts) occurred, most in all decades.
- As seen in the closing of gaps between each node, there was some international collaboration during this decade

*2000s*
- Noticeable increase in collaboration among countries, different coloured nodes are more concentrated in the middle
- Missions (with 2 or more astronauts) were embarked exclusively by other countries (*i.e.* without U.S. or Russian astronauts) only

*2010s*
- Reduced number of space missions 
- Nodes are more closely tied together, signifying more cross-country collaboration. For instance, Russian astronauts are found all around the network.
