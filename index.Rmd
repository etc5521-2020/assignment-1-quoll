...
# How can the report improve?

## Plots and tables
- In terms of the timeline, we can look at it through 'decades' instead of years to compare trends
- Improve figure captions that gives a brief summary of the plot
- Modify axis and legend names
- Set a common theme for the plots
- Add numerical summary that articulate the results of the plots
- Make plot colours more consistent
- Add comments to codes for better reproducibility

## Content
- In section 3, instead of talking only about gender discrepancy, we can dig deeper by exploring sociodemographic factors.
  - Are most astronauts in the military or civilian?
  - What are the occupations these astronauts?
  - These factors may tell us more about the difference in gender of astronauts.
- Improve introduction by telling reader some background of the astronauts

# New Question What are the trends for the collaboration among countries?
...
# Group year into decade, split the 'evolution through time' by decade
sample_cmbs <- sample_cmbs %>%
  ...
  mutate(mission_decade = case_when(
    ...
    between(year_of_mission, 2000, 2009) ~ "2000s", # International space station built in 1998
    between(year_of_mission, 2010, 2019) ~ "2010s")
  )

# Filter to nationalities that doesn't match
sample_cmbs %>%
  rowwise() %>%
  filter(nationality != with_nationality) 
```
